...
public class Simulation
{
    ...
    {
        if (espece == "Artichaut") Pot.ListePlantes.Add(new Artichaut(x, y))
        ...
        if (espece=="Tomate") Pot.ListePlantes.Add(new Tomatecv(x,y))
    }

    public void Planter()
    {
        if (Pot.SacDeGraine.Count == 0)
        {
        ...
        else
        {
            ...
            int x;
            while ((!Int32.TryParse(reponseX, out x)) || (x < 0) || (x >= Pot.Hauter))
            {
            ...
            SacDeGraines[numeroAPlanter].Quantite--;
            CreerPlante(SacDeGraines[numeroAPlanter].Espece, x, y);
        }
